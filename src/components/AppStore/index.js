...
class AppStore extends Component {
  state = {actId: tabsList[0].tabId, searchInput: ''}

  findTabApps = tabValue => {
    this.setState({actId: tabValue})
  }
  ...
  getFilterTabApps = () => {
    ...
    const filterTabApps = appsList.filter(
      eachTabList => eachTabList.category === actId,
    )
    return filterTabApps
  }
  ...
  render() {
    ...
    const filterTabApps = this.getFilterTabApps()
    const filterSearchApps = filterTabApps.filter(eachList =>
    ...
    return (
      <div className="bg">
        <div className="content-div">
          ...
          <ul className="table-div">
            {tabsList.map(eachTab => (
              <TabItem
                tabList={eachTab}
                key={eachTab.tabId}
                findTabApps={this.findTabApps}
              />
            ))}
          </ul>
          <ul className="app-item-div">
            {filterSearchApps.map(each => (
